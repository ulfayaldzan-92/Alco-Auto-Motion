...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  const handleNavClick = (tabId: 'input' | 'analysis' | 'edit_preview') => {
    onSelectTab(tabId);
  ...
  return (
    <>
      {/* Mobile Backdrop Overlay for safe drawer dismissal */}
      {!isCollapsed && (
        <div
      ...
      <aside
        ...
        className={`flex flex-col shrink-0 border-r border-border bg-sidebar transition-all duration-200 select-none ${
          isCollapsed
            ? 'hidden md:flex md:relative md:z-30 md:w-16'
            : 'fixed inset-y-0 left-0 z-50 w-64 shadow-xl md:shadow-none md:relative md:inset-auto md:z-30'
        }`}
      >
        {/* Brand Header */}
